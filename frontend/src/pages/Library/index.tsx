...
export default function Library() {
  ...
  const updateStatus = async (id: string, status: Item['status']) => {
    ...
    setItems((list) => {
      ...
      if (filter !== '') {
        return updated.filter((it) => (it.id === id ? status === filter : it.status === filter));
      }
      return updated;
    });
    try {
  ...
  const updateProgress = async (id: string, page: number) => {
    ...
    const prev = items;
    setItems((list) =>
      list.map((it) => (it.id === id ? { ...it, currentPage: Math.max(0, page) } : it)),
    );
      ...
      <div>
        <div className="mb-3 flex items-center gap-3">
          ...
          <select
            ...
            value={filter}
            onChange={(e) => setFilter(e.target.value as Item['status'] | '')}
          >
            ...
            {(() => {
              ...
              list.sort((a, b) => {
                switch (sort) {
                  case 'started_desc':
                    return (
                      ts(b.dateStarted) - ts(a.dateStarted) || ts(b.dateAdded) - ts(a.dateAdded)
                    );
                  case 'completed_desc':
                    return (
                      ts(b.dateCompleted) - ts(a.dateCompleted) || ts(b.dateAdded) - ts(a.dateAdded)
                    );
                  case 'title_asc':
                    return a.book.title.localeCompare(b.book.title, undefined, {
                      sensitivity: 'base',
                    });
                  case 'added_desc':
                  default:
              ...
              <div
                ...
                onClick={() => openDetails(it.book.id)}
              >
                ...
                <div className="flex items-center justify-between gap-2 mb-2">
                  <select
                    ...
                    onChange={(e) => updateStatus(it.id, e.target.value as Item['status'])}
                  >
                    <option value="READING">Reading</option>
                    <option value="COMPLETED">Completed</option>
                    <option value="BOOKMARKED">Bookmarked</option>
                ...
                {it.status === 'READING' && (
                  <div className="mt-1">
                    {(() => {
                      const total = it.book.pageCount || 0;
                      if (!total) return null;
                      const curr = it.currentPage || 0;
                      const pct = Math.min(100, Math.max(0, Math.round((curr / total) * 100)));
                      return (
                        <>
      ...
      {detailsId &&
        createPortal(
          ...
          >
            ...
            <div
              className="relative z-10 w-[40rem] max-w-[95vw] rounded-md border border-gray-200 bg-white p-6 text-sm shadow-lg dark:border-gray-700 dark:bg-gray-900"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex gap-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">
                    ...
                    {details?.firstPublishYear ? ` • ${details.firstPublishYear}` : ''}
                    {typeof details?.pageCount === 'number' ? ` • ${details.pageCount} pages` : ''}
                  </p>
                  {(() => {
                    const activeItem = items.find((it) => it.book.id === detailsId);
                    if (!activeItem) return null;
                    const added = fmt(activeItem.dateAdded);
                    const started = fmt(activeItem.dateStarted);
                    const completed = fmt(activeItem.dateCompleted);
                    return (
                  ...
                  {detailsLoading ? (
                    ...
                    <>
                      <div
                        className={
                          descExpanded
                            ? 'text-sm text-gray-800 dark:text-gray-200 max-h-40 overflow-auto pr-1'
                            : 'text-sm text-gray-800 dark:text-gray-200 line-clamp-6'
                        }
                  ...
                  <div className="mt-6 space-y-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateStatus(activeItem.id, e.target.value as Item['status'])
                        }
                      >
                        <option value="READING">Reading</option>
                        <option value="COMPLETED">Completed</option>
                    ...
                    {activeItem.status === 'READING' &&
                      (details?.pageCount || activeItem.book.pageCount) && (
                        <div>
                          ...
                          <div className="flex items-center gap-3 text-sm">
                            <button
                              ...
                              onClick={() =>
                                updateProgress(
                                  activeItem.id,
                                  Math.max(0, (activeItem.currentPage || 0) - 1),
                                )
                              }
                            >
                              <Minus size={12} />
                            ...
                            <button
                              ...
                              className="w-7 h-7 flex items-center justify-center rounded bg-gray-200 hover:bg-gray-300 disabled:opacity-50 dark:bg-gray-700 dark:hover:bg-gray-600"
                              onClick={() =>
                                updateProgress(activeItem.id, (activeItem.currentPage || 0) + 1)
                              }
                            >
                              <Plus size={12} />
                            </button>
                          ...
                          {(() => {
                            const total = (details?.pageCount ||
                              activeItem.book.pageCount ||
                              0) as number;
                            const curr = (activeItem.currentPage || 0) as number;
                            const pct = total
                              ? Math.min(100, Math.max(0, Math.round((curr / total) * 100)))
                              : 0;
                            return (
                              <div className="mt-2">
